...
test("health returns 200 with active, mode, relays and catalog 33", async () => {
 const port = 29180;
 ...
 try {
  ...
  assert.equal(direct.catalog, 33);
...
test("health with empty pool returns 200 and catalog 33", async () => {
 const port = 29182;
 const state = getActiveRelayState();
 const orig = JSON.parse(JSON.stringify(state));
