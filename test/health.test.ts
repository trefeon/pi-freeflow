...
test("health returns 200 with active, mode, relays and catalog 31", async () => {
	const port = 29180;
	...
	try {
		...
		assert.equal(direct.catalog, 31);
...
test("health with empty pool returns 200 and catalog 31", async () => {
	const port = 29182;
	const state = getActiveRelayState();
	const orig = JSON.parse(JSON.stringify(state));
