...
test("thin-provider lock: 33 models catalog intact", () => {
 const alive = getAliveCatalog();
 assert.equal(alive.length, 33, "catalog must be 33 (9 opencode + 19 kilo + 5 cline)");
 assert.equal(new Set(alive.map((m) => m.id)).size, 33);
...
test("thin-provider lock: proxy /v1/models pathname guard (no ?query leak)", async () => {
 ...
 try {
  ...
  assert.equal(json.data.length, 33);

  const qRes = await fetch(`http://127.0.0.1:${effectivePort}/v1/models?foo=bar`);
  assert.equal(qRes.status, 200, "query variant must be guarded to 200 with 33");
  const qJson = (await qRes.json()) as { data: Array<{ id: string }> };
  assert.equal(qJson.data.length, 33, "query variant must not leak paid models");
 } finally {
