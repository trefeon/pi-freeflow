...
test("thin-provider lock: 24 models catalog intact", () => {
	const alive = getAliveCatalog();
	assert.equal(alive.length, 24, "catalog must be 24 (7 opencode + 17 kilo)");
	assert.equal(new Set(alive.map((m) => m.id)).size, 24);
	assert.equal(ALL_MODELS.length, 24);
});
...
test("thin-provider lock: proxy /v1/models pathname guard (no ?query leak)", async () => {
	...
	try {
		...
		assert.equal(qRes.status, 200, "query variant must be guarded to 200 with 24");
		const qJson = (await qRes.json()) as { data: Array<{ id: string }> };
		assert.equal(qJson.data.length, 24, "query variant must not leak paid models");
	} finally {
