 ...
 * ports 28180/18080:
 *   1. Fresh install — activation registers 34 models, binds a proxy, onboards
...
test("user flow [1/6] fresh install registers 34 models, binds a proxy, and onboards exactly once", async () => {
	...
	> = {};
		...
		try {
			...
			assert.equal(
				...
				ALL_MODELS.length,
				"all 34 static models must be registered",
			);
			...
			assert.match(
				msgs[0],
				/^freeflow ready: 34 free models via local proxy 127\.0\.0\.1:28180\. /,
