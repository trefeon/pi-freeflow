 ...
 * ports 28180/18080:
 *   1. Fresh install — activation registers 32 models, binds a proxy, onboards
...
test("user flow [1/6] fresh install registers 32 models, binds a proxy, and onboards exactly once", async () => {
	...
	> = {};
		...
		try {
			...
			assert.equal(
				...
				ALL_MODELS.length,
				"all 32 static models must be registered",
			);
			...
			assert.match(
				msgs[0],
				/^freeflow ready: 32 free models via local proxy 127\.0\.0\.1:28180\. /,
